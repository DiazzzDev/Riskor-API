...
public class ControllerInspection {
    ...
    @GetMapping("/getInspectionByTitle/{title}")
    public ResponseEntity<?> getInspectionByTitle(
            @RequestAttribute("auth.business") String idBusiness,
        ...
        try {
            ...
            Page<DTOInspection> regulations = objServiceInspection.getInspectionsByTitle(page, size, title, idBusiness);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of(
                    "status", "Error crítico no controlado",
                    "message", "Error al listar inspecciones",
                    "detail", e.getMessage()
            ));
        }
    }
